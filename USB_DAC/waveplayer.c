...
{ 
  ...
  /* Start playing wave */
  Audio_MAL_Play((uint32_t)DACBuffer, _MAX_SS);
    ...
    { 
      ...
      if(buffer_switch == 0)
      {
        ...
        Audio_MAL_Play((uint32_t)DACBuffer, _MAX_SS);
    	  ...
    	  DMA_Configuration(&buffer1, _MAX_SS);
    	  DMA_Cmd(DMA1_Stream5, ENABLE);
      ...
      {   
        ...
        Audio_MAL_Play((uint32_t)DACBuffer, _MAX_SS);
		DMA_Cmd(DMA1_Stream5, DISABLE);
		DMA_Configuration(&buffer2, _MAX_SS);
		DMA_Cmd(DMA1_Stream5, ENABLE);
...
/**
  ...
  * @retval None
  */
void WavePlayerStart(int filenumber)
{
	filenames[0] = "0:file1.wav";
	filenames[1] = "0:file2.wav";
	filenames[2] = "0:file1.wav";
  ...
  else
  {
    if (WaveRecStatus == 1)
    {
      WaveFileName = filenames[filenumber];
    }
    else
    {
    WaveFileName = filenames[filenumber];
